...
spelev <- read.table("./forestplot_160_spatial_data.txt",header = T)
# coordinates 
...
FunBoots <- function(x){
  ...
  coefVals <- data.frame(int=c(cf2[1]+cf2[4:19]), 
                         x1=c(cf2[2]+cf2[20:35]), 
                         x2=rep(cf2[3], each=16),
                         sp=sp)
...
post.samp2 <- foreach(i = 1:5000, .combine="cbind") %do% FunBoots() # foreach loop, cbind to store 
