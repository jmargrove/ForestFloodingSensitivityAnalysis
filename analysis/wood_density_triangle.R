...
p2 <- ggplot(data = spat_data, aes(x = d, y = e)) + 
...
with(spat_data, plot(y = e, x = d))
mdist <- matrix(mdist, nrow = 2)
mCI025 <- matrix(vCI025, nrow = 2)
mCI975 <- matrix(vCI975, nrow = 2)
for(i in 1:5){
  abline(a = mdist[1, i], b = mdist[2, i], col = "red")
}


preds <- expand.grid(d = seq(min(spat_data$d), max(spat_data$d), length = 100), taus = taus)

preds$e <- as.vector(foreach(i = 1:5, .combine = cbind) %do% linerEQ(mdist[1, i], mdist[2, i],  seq(min(spat_data$d), max(spat_data$d), length = 100)))
preds$CI025 <- as.vector(foreach(i = 1:5, .combine = cbind) %do% linerEQ(mCI025[1, i], mCI025[2, i],  seq(min(spat_data$d), max(spat_data$d), length = 100)))
preds$CI975 <- as.vector(foreach(i = 1:5, .combine = cbind) %do% linerEQ(mCI975[1, i], mCI975[2, i],  seq(min(spat_data$d), max(spat_data$d), length = 100)))




ggplot(preds, aes(x = d, y = e, group = taus)) + geom_line() + 
  geom_ribbon(aes(ymin = CI025, ymax = CI975), alpha = 0.1)
...
CI <- booter(model = rmod, data = spat_data, preds = pred, n = 1000, quantReg = TRUE)
preds$CI025 <- CI[1,]
preds$CI975 <- CI[2,]
